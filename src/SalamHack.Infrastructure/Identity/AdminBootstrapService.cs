    ...
    : IHostedService
{
    ...
    private static async Task EnsureRoleExistsAsync(RoleManager<IdentityRole<Guid>> roleManager)
    {
    ...
    private async Task EnsureFirstAdminAsync(UserManager<ApplicationUser> userManager)
    {
        var admins = await userManager.GetUsersInRoleAsync(ApplicationConstants.Roles.Admin);
        if (admins.Count > 0)
        {
        ...
        if (!await userManager.IsInRoleAsync(user, ApplicationConstants.Roles.Admin))
        {
            var roleAssignResult = await userManager.AddToRoleAsync(user, ApplicationConstants.Roles.Admin);
