...
public class VerifyRegistrationCommandHandler(
    ...
    TimeProvider timeProvider) : IRequestHandler<VerifyRegistrationCommand, Result<AuthResponse>>
{
    private readonly RefreshTokenSettings _rtSettings = rtOptions.Value;

    public async Task<Result<AuthResponse>> Handle(
        VerifyRegistrationCommand request, CancellationToken ct)
    {
        var verificationResult = await emailVerificationService.VerifyRegistrationOtpAsync(
            request.Email,
            request.Otp,
            ct);
