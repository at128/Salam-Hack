...
internal sealed partial class CookieSettingsValidator : IValidateOptions<CookieSettings>
{
    public ValidateOptionsResult Validate(string? name, CookieSettings options)
    {
        ...
        {
            failures.Add("CookieSettings:RefreshTokenCookieName contains invalid characters.");
        }

        if (options.RefreshTokenExpiryDays is <= 0 or > 90)
            failures.Add("CookieSettings:RefreshTokenExpiryDays must be between 1 and 90.");
        ...
        {
            failures.Add("CookieSettings:SameSite must be one of: Strict, Lax, None.");
        }
        else if (sameSite == SameSiteMode.None && !options.SecureOnly)
        {
            failures.Add("CookieSettings:SecureOnly must be true when SameSite is None.");
        }
